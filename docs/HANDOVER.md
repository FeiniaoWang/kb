...
**Next up: `kb create` (CLI-13).** The immediate task is the contract spec for `kb create`, the deterministic write path for **synthetic** documents (the counterpart to `kb ingest` for raw). Its design is already fleshed out in the master design and PRD (see §3/§7); the session's job is to turn that into a full contract spec mirroring the existing ones.
...
**For `kb create` (next) — master design bullets are extensive (see the CLI-13 section); genuinely open sub-details to settle in-session:**
...
2. Run the §4 process: clarifying questions (one at a time) on the genuinely open details listed in §7 for `kb create`, then sectioned design presentation, then write `docs/specs/commands/kb-create.md`, self-review, and hand to the human for the review gate.
3. Update 00-shared §7 (models introduced/graduated), the master design, and the PRD wherever the new spec sharpens or changes behavior — then verify consistency by grepping across all documents before declaring done.
