...
| E14 | OS error during context acquisition, preparation, or the write phase | `E_CREATE_IO` with the OS message; pre-flight failures leave the KB untouched, while write-phase failures may leave completed earlier effects (documented, not rolled back) | 2 |
