...
2. Scan (00-shared §5). If `KB.malformed` is non-empty → `E_INGEST_MALFORMED`, exit 2, naming the malformed paths and directing to `kb validate` — the id-allocation guard of 00-shared §6.
   ...
   Context, malformed, surface, destination, and adapter-acquisition order is
   unchanged. If rooted context acquisition itself fails at a path proven to
   be one of the requested class/destination components, that existing failure
   is classified as `E_INGEST_DEST_INVALID` with the normal destination
   message. For this comparison only, ingest parses `--dest` without raising:
   invalid syntax proves no match. It constructs the lexical root-relative
   prefix chain from the selected class directory through the requested
   destination and requires exact equality with the typed unsafe path. It does
   not invoke the adapter, perform an independent filesystem read, or move
   destination traversal ahead of context loading. Any unrelated context
   failure remains its generic/config/malformed classification; in particular,
...
14. Append the log entry (00-shared §8) using only the exact bytes produced during step 10: action `ingested`, actor `--actor` (default `kb-cli`), ids = the new id, note = `<document relpath> from <origin>`. The write phase performs no log text formatting or encoding. A missing `log.md` is first recreated with the kb-init §5.3 scaffold (frontmatter + header comment + heading, no `initialized` line) and then appended.
...
| E6 | `--dest` empty; absolute; containing a backslash or `.`/`..` segment; using a Windows drive or UNC spelling; containing any existing symlink, junction, or non-directory component from the class directory through the target (including links that stay in-class); escaping the class directory; or rooted context acquisition fails at an unsafe path exactly matching that lexical class/destination component chain | `E_INGEST_DEST_INVALID` naming the offending value, before external acquisition; exact context provenance is comparison-only and does not add an earlier traversal | 2 |
...
| E12 | OS error during context acquisition, preparation (including log formatting/UTF-8 encoding), or the write phase, except the exact destination-component context case in E6 | `E_INGEST_IO` with the OS message; pre-flight failures leave the KB untouched, while write-phase failures may leave completed earlier effects (documented, not rolled back) | 2 |
