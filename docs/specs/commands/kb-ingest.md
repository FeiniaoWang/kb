...
9. Compute names:
   ...
   - **Collision:** treat the candidate as occupied if the document path — or, non-text, the original path — already exists in the target directory, **or** if the target directory will be created and the document candidate is its implicit CLI-maintained `index.md`. Append `-<lowercased id>` to **both** stems (`meeting-notes.md` → `meeting-notes-raw-000042.md`, `index.md` → `index-raw-000042.md`). For a non-text `.md` source the pair becomes `<slug>-<lowercased id>.md` and `<slug>-<lowercased id>.md.original`. Reserving the implicit path keeps the citable stub distinct from step 11's born-current directory index and keeps a non-text companion visibly paired with its suffixed stub. The id is unique under a valid pre-flight scan; if either exclusive creation nevertheless finds a late occupant, ingestion fails with `E_INGEST_IO`. Nothing existing or implicit is ever overwritten or followed (raw is append-only, LS4).
...
| E14 | Filename collision from an existing document/original, identical re-ingest, or a candidate `index.md` reserved for a target directory that will be created | allowed — new id, collision-suffixed document filename and paired companion stem where applicable; no de-duplication or overwrite | 0 |
