...
**Traceability:** PRD CLI-6, CLI-12/NFR-3 (deterministic mechanical truth — every property checkable without an LLM is checked here), FM0 (universal `type`), FM1/FM1a/FM1b (per-class schema, description length, tag vocabulary), FM2 (unknown keys never flagged), FM3 (mechanical checkability), DG1 (≥1 parent), DG2 (acyclicity/termination), DG5 (session parentage), LS1 (legal status values), LS2 (timestamp ordering), LS4 (supersedes/status coupling), NFR-2 (OKF conformance), NFR-7 (merge-collision detection), NFR-9 (stable finding codes as a machine contract). The `kb-lint` skill's semantic review (LINT-2, LINT-3) is out of scope (§9).

## 1. Purpose

Check the knowledge base's mechanical integrity: `kb validate` is the KB's **single authoritative integrity checker** and the CI gate (NFR-3, CLI-12) — the enforcement point for every rule the write paths deliberately deferred (full per-class schema, tag vocabulary, type↔location agreement, id uniqueness, link resolution, graph acyclicity, DG5 session parentage, supersedes/status coupling). It is **total**: it never stops at the first problem — every check runs over every in-scope file and all findings are reported in one pass. It is **read-only**: the KB is byte-for-byte unchanged, and no log entry is written. Every finding carries a **stable code** (the machine contract CI and skills key off, NFR-9), a severity (`error`/`warning`), the anchoring file, and a message. Errors are the CI gate (exit 1); warnings advise (exit 0 unless `--strict`). Judgment — contradictions, staleness, orphan analysis, drift — belongs to the `kb-lint` skill, never to this command.
...
4. **Run every check over the whole universe** (scope never disables a check — graph and uniqueness checks are inherently global). The checks, their trigger conditions, severities, and messages are pinned in §7.1. Cross-cutting rules:
   ...
   - **Graceful degradation.** Checks never cascade off broken inputs: graph checks (DG1/DG2/DG5) run only where `derived_from` is a well-shaped list of strings (otherwise `FM1_FIELD_INVALID` stands alone); `DG1_NO_PARENTS` preempts `DG5_NO_SESSION_PARENT` (an empty parent list is one defect, not two); DG5 evaluates over the parents that resolve (unresolvable ones already carry `LINK_UNRESOLVED`); the `ID_INVALID`/`ID_PREFIX_MISMATCH` checks evaluate only string ids on classes whose §4.1 schema includes `id` (a non-string `id` is `FM1_FIELD_INVALID` alone; an `id` on an `index.md` is `FM1_KEY_FORBIDDEN` alone — but `ID_DUPLICATE` compares every literal string `id` in the KB regardless of class); `LS_TOUCH_AFTER_TIMESTAMP` compares only timestamps that parse; `LS_SUPERSEDES_NOT_SYNTHETIC`/`LS_SUPERSEDES_NOT_MARKED` evaluate only a resolved, non-self target (`LS_SUPERSEDES_SELF` preempts both).
   - **Keys are checked only where legal.** The link (`LINK_UNRESOLVED`) and lifecycle (`LS_*`) checks evaluate `derived_from`/`supersedes` on synthetic documents and `about` on feedback documents — where the §4.1 schema defines them. On any other class the same key is `FM1_KEY_FORBIDDEN` and its value is not otherwise evaluated.
   - **Graph edges.** The derivation graph's edges are the resolvable `derived_from` entries of **synthetic** documents only. Raw, governance, and index documents contribute no edges (`derived_from` on them is `FM1_KEY_FORBIDDEN`). With DG1, `LINK_UNRESOLVED`, and `DG2_CYCLE` in force, DG2's termination clause — every chain terminates at raw or governance documents — is a theorem, not a separate check: every synthetic document has parents, every edge lands on an existing document, no path cycles, so every maximal chain ends at a non-synthetic document. It has no finding code.
...
- **Reserved slug id:** `GOVERNANCE-<SLUG>` where `<SLUG>` is an uppercase letter followed by uppercase letters, digits, or hyphens (e.g. `GOVERNANCE-CONVENTIONS`).
- **Reserved schema keys** (the FM1 key namespace; everything outside it is an FM2 extension key, preserved and **never flagged**): `id`, `type`, `title`, `description`, `status`, `derived_from`, `timestamp`, `last_human_touch`, `tags`, `supersedes`, `instructions`, `ingested_at`, `origin`, `about`.
...
|---|---|---|
| synthetic | `id`, `title`, `description`, `status`, `derived_from`, `timestamp`, `last_human_touch`, `tags`, `supersedes`, `instructions` | `id`, `title`, `description`, `status`, `derived_from`, `timestamp`, `last_human_touch` |
...
| governance (`conventions`, `kb-config`, `health`) | `id`, `title`, `description` | `id`, `title`, `description` |
...
- **Field shapes** (`FM1_FIELD_INVALID`): `id`, `title`, `description`, `origin`, `instructions`, `supersedes`, `about` — non-empty strings (empty or whitespace-only is invalid); `status` — literally one of `draft`, `current`, `superseded`, `retired` (case-sensitive); `derived_from`, `tags` — lists whose entries are all non-empty strings (an empty `tags` list is legal; an empty `derived_from` is well-shaped but triggers `DG1_NO_PARENTS`); `timestamp`, `last_human_touch`, `ingested_at` — valid timestamps as defined above.
- **Vocabulary checks:** `TAG_UNDECLARED` fires for every `tags` entry absent from the config `tags` list — including when that list is empty ("an empty list means no tags are declared", GOVERNANCE-KB-CONFIG). `TYPE_UNDECLARED` fires for a synthetic `type` absent from a **non-empty** config `types` list; an empty `types` list means the type vocabulary is not yet constrained and no finding fires (same source). This asymmetry follows the governance reference verbatim.
...
- `checked` — number of in-scope files. `findings` — same order as the text lines; `id` is the file's literal frontmatter `id` when present as a string, else `null` (malformed, index, log). `counts` counts the reported findings by severity. `unresolved_refs` — the unresolvable ref tokens in argument (then stdin) order, `[]` when none.
...
| `LOC_TYPE_MISMATCH` | error | location disagrees with the authoritative `type`: `index` → the file must be named `index.md`; `raw-source`/`chat`/`feedback` → under `raw/sources/`/`raw/chats/`/`raw/feedback/` (any depth); governance types → under `governance/` (any depth); `log` → exactly the root `log.md`; any synthetic type → under `synthetic/` (any depth); **and conversely** a file named `index.md` must carry `type: index` | `type '<type>' does not agree with location '<path>' (expected <expectation>)` |
...
| `FM1_FIELD_INVALID` | error | a reserved schema key present with the wrong §4.1 shape (illegal `status` value, non-list `derived_from`/`tags`, non-string or empty string field, unparseable timestamp) | `field '<field>' is invalid: <reason>` |
| `FM1_KEY_FORBIDDEN` | error | a reserved schema key present on a class whose §4.1 legal set does not include it (e.g. `about` outside feedback, `status`/`derived_from` on a raw document, `id` on an `index.md`). FM2 extension keys (outside the reserved set) are never flagged | `key '<key>' is not part of the <class> schema` |
...
| `ID_DUPLICATE` | error | the same literal `id` string on two or more files — one finding **per participant**, naming the others (merge collisions, NFR-7; repair is manual) | `id '<id>' is also carried by <other paths, alphabetical, comma-separated>` |
| `LINK_UNRESOLVED` | error | a `derived_from` entry, `supersedes` value, or `about` value that is not an existing document's id (one finding per value; link values are canonical ids — a path never resolves here, and an id hidden in a malformed file does not exist) | `<field> reference '<value>' does not resolve to a document id` |
...
Twenty codes: sixteen errors, four warnings (`FM1_DESCRIPTION_LONG`, `TYPE_UNDECLARED`, `TAG_UNDECLARED`, `LS_TOUCH_AFTER_TIMESTAMP`).
...
**Coverage map:** clean KBs & read-only AC1–AC5 (§1, §4 step 2, §5, E9/E10) · FM0 AC6–AC10 (§4 step 4, E6) · location AC11–AC15 (§7.1 `LOC_TYPE_MISMATCH`) · schema: missing fields AC16–AC20 (§4.1 mandatory table) · schema: invalid shapes AC21–AC25 (§4.1 field shapes) · forbidden keys & FM2 AC26–AC28 · description & vocabularies AC29–AC33 (§4.1 heuristics/vocabulary rules) · ids AC34–AC39 (§4.1 id forms, E16) · links AC40–AC43 · graph AC44–AC49 (DG1/DG2/DG5, termination theorem) · lifecycle AC50–AC54 (LS codes) · severity, `--strict` & exit codes AC55–AC57 (E11/E12) · scoping & pipelines AC58–AC64 (§4 steps 3/5, E5/E7/E13) · output & JSON AC65–AC69 (§6) · environment, usage, help & Git AC70–AC74 (E1–E4, §3, 00-shared).

| # | Given / When / Then |
|---|---|
| AC1 | Given a fresh `kb init` scaffold, when `kb validate` runs, then exit 0 and stdout is exactly `no findings — checked 11 files` (8 `index.md` + 2 governance documents + `log.md`) — the scaffold is born valid (E10; flips kb-init AC30 from xfail to a hard gate). |
...
| AC12 | Given a synthetic-typed document at the KB root (case a) and a `type: conventions` document under `synthetic/` (case b), then each yields `LOC_TYPE_MISMATCH`. |
...
| AC20 | Given a governance document (`type: conventions`) missing `description` (case a) and an `index.md` missing `description` (case b), then each yields `FM1_FIELD_MISSING`. |
...
| AC36 | Given a governance document with `id: GOVERNANCE-000001` (numeric on governance) (case a) and one with `id: CONV-CONVENTIONS` (wrong slug prefix) (case b), then each yields `ID_INVALID`; `GOVERNANCE-CONVENTIONS` (case c) and a synthetic `KB-1000001` (case d) yield nothing. |
...
| AC43 | Given a synthetic document with `derived_from: [GOVERNANCE-CONVENTIONS, CHAT-000001]`, then no `LINK_UNRESOLVED` — reserved slug ids resolve like any id. |
...
- **Semantic review** — the `kb-lint` skill's judgment layer (LINT-2/LINT-3), strictly excluded by NFR-3: contradictions between `current` documents, staleness relative to ancestors, orphan/coverage analysis (DG3), `instructions` conflicts, and the drift report (`current` documents changed since `last_human_touch`). `kb validate` supplies the mechanical findings `kb-lint` builds on.
...
- **`status: superseded` with no inbound `supersedes` link** — not mechanically wrong (legitimate pre-merge and adoption states exist); lineage review is lint territory (E15's counterpart).
