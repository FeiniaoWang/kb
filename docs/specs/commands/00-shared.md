...
| `SyntheticFrontmatter` | `core/model.py` | `id`, `type`, `title`, `description`, `status` (draft\|current\|superseded\|retired), `derived_from: list`, `timestamp`, `last_human_touch`; optional `tags`, `supersedes`, `instructions`. **Constructed and emitted by kb-create** in exactly this key order (kb-create §5.1; optional keys last, only when present; `derived_from`/`tags` as block sequences); cross-KB enforcement specced in kb-validate | kb-create (emission), kb-validate (enforcement) |
