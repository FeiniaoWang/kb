...
| `OperationalFrontmatter` | `core/model.py` | `type: log`; no other reserved schema key is legal, while unknown FM2 extension keys are preserved. Used by the root path-addressed operational document `log.md` (`DocClass OPERATIONAL`), whose body is CLI-appended in the §8 format | kb-init |
| `Document` | `core/model.py` | `id: str \| None` (a canonical id only for id-bearing raw, synthetic, and governance classes; `None` for invalid/noncanonical ids and for index/operational documents; numeric ids parse via `DocId`), `path` (KB-root-relative), `doc_class: DocClass`, `frontmatter: Frontmatter`, `body` (lazy property) | kb-ingest |
