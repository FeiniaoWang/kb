...
- `<LISTING>` is one `- <name> — <its description>` line per **immediate child** — subdirectories then child documents, each alphabetical — sourced from each child's `description`. Operational files (`log.md`, `kb-config.json`) and the directory's own `index.md` are never listed. A leaf directory with no children has an empty `<LISTING>` (the generated-header comment, then nothing).
- `<DESCRIPTION>` is set at directory creation (pinned below for the scaffold; a default is generated for directories created later by `kb ingest`) and is CLI-owned. Blank-line layout: one blank line after `# <TITLE>`, the generated-header comment, one blank line, then the listing.

Per-directory pinned values:

| Path | `<TITLE>` | `<DESCRIPTION>` | `<LISTING>` |
|---|---|---|---|
| `index.md` | `Knowledge Base` | `Root of the knowledge base — raw evidence, synthetic documents, and governance.` | `- governance/ — Project governance: configuration, conventions, and templates.`<br>`- raw/ — Immutable evidence: sources, session records, and feedback.`<br>`- synthetic/ — Human-created, agent-maintained documents.` |
| `governance/index.md` | `Governance` | `Project governance — configuration, conventions, and templates.` | `- templates/ — Optional per-type document templates.`<br>`- conventions.md — Standing project conventions.`<br>`- kb-config.md — Field reference for kb-config.json.` |
| `governance/templates/index.md` | `Templates` | `Optional per-type document templates.` | (empty) |
| `raw/index.md` | `Raw Evidence` | `Immutable evidence — sources, session records, and feedback.` | `- chats/ — Session records from kb-author / kb-ingest.`<br>`- feedback/ — Consumer feedback about synthetic documents.`<br>`- sources/ — Normalized external evidence.` |
| `raw/sources/index.md` | `Sources` | `Normalized external evidence (immutable).` | (empty) |
| `raw/chats/index.md` | `Session Records` | `Session records from kb-author and kb-ingest sessions (immutable).` | (empty) |
| `raw/feedback/index.md` | `Feedback` | `Consumer feedback about synthetic documents (immutable).` | (empty) |
| `synthetic/index.md` | `Synthetic Documents` | `Human-created, agent-maintained documents (the derivation graph).` | (empty) |

When `<LISTING>` is empty, the section is: frontmatter, `# <TITLE>`, one blank line, the generated-header comment, then no listing. Every scaffolded `index.md` is byte-for-byte fixed (no timestamps).
