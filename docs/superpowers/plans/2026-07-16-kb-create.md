...
The final whole-branch reviews added RED→GREEN safety cases that supersede
unsafe assumptions in the sample implementation above:

- **Flow-map lifecycle edits:** cover flow mappings missing one or both
  timestamps, trailing flow separators (including a separator followed by a
  comment and newline), and explicit YAML document-end markers; detect reusable
  separators from token order, insert inside the top-level mapping, and reparse
  the fully prepared frontmatter before any write.
- **Canonical destination containment:** reject backslashes and every existing
  symlink or junction in `synthetic/` and destination ancestors, then verify
  resolved containment beneath the real synthetic root before writes.
- **Exclusive document creation:** race a file into place at the low-level open
  boundary and require `O_CREAT|O_EXCL` (`O_NOFOLLOW` where available) to return
  `E_CREATE_IO` without changing the raced-in bytes.
- **Existing mutable-file no-follow:** reject symlink/junction final paths for
  the superseded document, regenerated existing index, and root log during
  preflight; open with `O_NOFOLLOW` where available and compare `fstat`
  device/inode identity before truncate, write, or append. Create a missing log
  exclusively so a raced-in path is never followed or overwritten.
- **Missing synthetic base:** delete `synthetic/` and cover both default and
  nested destinations; create born-current indexes for the full missing chain
  and regenerate only the root as the nearest pre-existing ancestor.
- **Prefix defaults and validation:** cover partial `id_prefixes` default merging
  plus lowercase, malformed, non-string, and reserved `GOVERNANCE` values in
  text and JSON; reject invalid config before writes while preserving valid
  custom prefixes.
