...
The final whole-branch review added five RED→GREEN safety cases that supersede
unsafe assumptions in the sample implementation above:

- **Flow-map lifecycle edits:** cover flow mappings missing one or both
  timestamps, trailing flow separators, and explicit YAML document-end markers;
  insert inside the top-level mapping and reparse the fully prepared frontmatter
  before any write.
