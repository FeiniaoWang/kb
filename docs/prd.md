...
| Field | Value |
|---|---|
| **Status** | Draft v0.4 (supersedes v0.3) |
...
**Consistency updates (2026-07-14), from the CLI design pass ([docs/superpowers/specs/2026-07-13-kb-cli-design.md](superpowers/specs/2026-07-13-kb-cli-design.md), [docs/specs/commands/](specs/commands/)):** Project config is `kb-config.json` — strict JSON at the **KB root**, which also serves as the root-discovery marker; it moved out of `governance/`. Document ids zero-pad to **6 digits** (`KB-000042`). Added **`kb show`** (CLI-3a) as the sole sanctioned document-read path for skills, and `kb frontmatter --field` for single-field retrieval. Id↔path resolution is by per-invocation scan, with no stored index. The CLI performs **no Git operations** (Git is the user's/skills' responsibility). Governance system files carry reserved slug ids (`GOVERNANCE-CONVENTIONS`, `GOVERNANCE-KB-CONFIG`). **Every directory has an `index.md`** (a first-class `DocClass INDEX` document, CLI-maintained and read-only for humans/agents); `.gitkeep` files are gone, and `kb index` regenerates each `index.md` listing.

**Consistency updates (2026-07-15), from the `kb ingest` spec ([docs/specs/commands/kb-ingest.md](specs/commands/kb-ingest.md)):** CLI-8's synopsis gains an explicit `SOURCE` positional (the input file, required with `--from file`) and `--dest` (target subdirectory under the class directory). Raw frontmatter gains an always-written `title` (used in `index.md` listings; not part of FM1's mandatory set). The clipboard adapter shells out to platform tools; filename collisions are auto-suffixed with the new id; ingest validates fully before writing, so a failed run leaves the KB untouched.

**Consistency updates (2026-07-16):** Added **`kb create`** (CLI-13) — the deterministic write path for **synthetic** documents, the counterpart to `kb ingest` for raw. It allocates the `KB-` id, emits schema-valid synthetic frontmatter (`id`, `type`, `title`, `description`, `status`, `derived_from`, `timestamp`, `last_human_touch`, optional `tags`/`supersedes`/`instructions`), files the document under `synthetic/`, refreshes `index.md`, and appends a `created` log entry — validating fully before writing so a failed run leaves the KB untouched. This closes the gap where synthetic id allocation and frontmatter had no CLI command and would otherwise fall to skill judgment, keeping mechanical correctness CLI-owned (NFR-3/§5.2). `kb create` enforces DG1 (≥1 parent) but not DG5 (session-record parent), which `kb validate` continues to own; `--supersedes` atomically marks the prior document `superseded` — its target must be a live (`draft`/`current`) synthetic document, and it is implicitly counted among the new document's parents (satisfying DG1 by itself). In-place **revision** of existing synthetic documents remains unspecified (new OQ6). `kb-author`'s AUT-6/AUT-8 now invoke `kb create`.

**Consistency updates (2026-07-16), from the `kb validate` spec ([docs/specs/commands/kb-validate.md](specs/commands/kb-validate.md)):** CLI-6 is now fully contracted: every violation is a **finding** with a stable, family-prefixed code (20 codes: 16 errors, 4 warnings), severity, anchoring path, and normative message — a namespace separate from `E_*` command errors, and the machine contract for CI and skills (NFR-9). The warnings are exactly: `description` > 2 sentences (FM1a), `last_human_touch` > `timestamp`, undeclared tag (FM1b), and undeclared synthetic type; per GOVERNANCE-KB-CONFIG, an empty `types` list means the vocabulary is unconstrained (no finding) while an empty `tags` list means no tags are declared (every tag flags). `--strict` promotes warnings into the exit-1 signal only; scoping (`kb validate REF...`) filters which findings are reported, never which checks run, with multi-document defects (duplicate ids, cycles) emitting one finding per participant. Sharpened: DG2's termination clause is a theorem given DG1 + link resolution + acyclicity (no finding code of its own); numeric ids must be canonical (`KB-0000042` is invalid, closing its ambiguity with `KB-000042`); known schema keys on the wrong class are findings (`FM1_KEY_FORBIDDEN`) while FM2 openness covers only genuinely unknown keys. Directory-invariant presence and index freshness belong to `kb index --check` (the CI gate composes `kb validate && kb index --check`).

**Changes from v0.3 (2026-07-18), from the vision-refinement grilling session ([CONTEXT.md](../CONTEXT.md), [ADRs 0001–0004](adr/)):** Purpose becomes explicit: a governance **KB charter** (`governance/charter.md`, reserved id `GOVERNANCE-CHARTER`) records the KB-level purpose that shapes structure and vocabularies; **purpose documents** — clean-context leaves distilled from a small sub-network for one specific outcome — are named as the KB's end product, with **building blocks** as the intermediate nodes (§1, §6.9, OQ7). **Associative links** added (§6.9): a governance-declared link-type vocabulary (shipped defaults `references`, `contradicts`, `constrains`) as synthetic-side frontmatter — raw stays fully immutable (ADR 0002) — that notify on upstream change but never obligate propagation (ADR 0004). **In-place revision** (`kb revise`, CLI-14) resolves OQ6: consolidation of new evidence preserves the document id; supersession is reserved for genuine replacement (ADR 0001). **Consolidation** flow defined (CS6): the human declares at ingest which documents new evidence concerns; the agent proposes changes and the human confirms in-session. Deferred propagation marks children with a **`pending_upstream`** frontmatter field, never a status change; consumers of marked documents are warned, not blocked (ADR 0003). The data-lake→warehouse analogy is adopted as explanatory framing for human-led distillation — explicitly not an automated pipeline (G2/NG3 unchanged).
...
pending_upstream: [KB-000007]    # present only while a parent's revision awaits propagation (CS6/ADR 0003)
...
- **AL2.** Associative links live on **synthetic documents only**; raw immutability is fully preserved (ADR 0002). A raw↔raw relationship (a new note contradicting last month's) is expressed where it matters — in the downstream synthetic documents, where the later evidence overrides the earlier on conflict (CS1).
- **AL3.** Associative links never satisfy DG1/DG5 parentage and never create propagation obligations. When a link target is revised or superseded, linking documents surface in the impact report as *aware of change* — notified, never obligated; no `pending_upstream` marking (ADR 0004). They are likewise exempt from DG2: cycles among associative links (mutual `references`) are legal, and they never count toward derivation-chain termination.
...
| **Graph** | **CLI-4.** `kb links <id|path>` — parents; `--reverse` — children; `--transitive` — full ancestry/impact set; `--depth` — derived depth-from-evidence; traverses `derived_from` by default and associative links (§6.9) on request (AL4; flag per the command spec). **CLI-5.** `kb resolve <id>` / reverse — id↔path resolution computed by the per-invocation scan (no stored index; §6.5). |
| **Integrity** | **CLI-6.** `kb validate [REF…] [--strict]` — the single authoritative integrity checker: universal `type` and type↔location agreement (FM0/NFR-2), per-class frontmatter schema (FM1–FM3), type/tag/link-type vocabularies (FM1b, §6.9), id format/uniqueness (including merge collisions), resolution of provenance and associative links, graph acyclicity (DG2), parentage (DG1/DG5), and supersedes/status coupling. Read-only and total (reports every finding in one pass); each finding carries a stable code and severity (`error` → exit 1, the CI gate; `warning` → exit 0 unless `--strict`); scoping filters reported findings, never checks. Full contract: [kb-validate.md](specs/commands/kb-validate.md). **CLI-7.** `kb mv` — move/rename preserving id, updating body links. |
| **Ingest adapters** | **CLI-8.** `kb ingest --class source|chat|feedback --from file|stdin|clipboard [SOURCE] [--dest <subdir>] [--about <ref>]` — normalize incoming material into the matching `raw/` subclass with correct frontmatter and assigned ids (`--about` links a feedback record to the document it concerns); a `--dest` subdirectory that does not yet exist is created together with its `index.md`. Adapter architecture is extensible (Slack, mail, ticketing later) without skill changes. Adapters normalize and file; they never synthesize. |
| **Authoring** | **CLI-13.** `kb create --type T --title TITLE --description DESC [--dest <subdir>] [--derived-from REF]… [--status draft|current] [--tag G]… [--supersedes REF] [--instructions TEXT] [--body-file FILE|-]` — the deterministic write path for **synthetic** documents (counterpart to `kb ingest` for raw): allocates the next `KB-` id, emits schema-valid synthetic frontmatter, files the document under `synthetic/` (creating `--dest` subdirectories with their `index.md`), sets `timestamp` and `last_human_touch` to the creation instant, refreshes the target `index.md`, and appends a `created` log entry. Requires ≥1 parent (DG1): `--derived-from` refs stored as canonical ids in argument order, and/or `--supersedes`, whose target — a live (`draft`/`current`) synthetic document — is implicitly added to `derived_from`; `--supersedes` atomically marks the prior document `superseded`. Validates fully before writing — a failed run leaves the KB untouched. Agents never hand-write synthetic files; id allocation and frontmatter schema stay CLI-authoritative. Session-record parentage (DG5) is enforced by `kb validate`, not by this command. **CLI-14.** `kb revise REF [--add-parent REF]… [--link TYPE=REF]… [--body-file FILE|-] [--status draft|current|retired] [--clear-pending REF]…` — in-place revision of a synthetic document (resolves OQ6, ADR 0001): preserves the id; updates body/frontmatter; appends parents (consolidation, CS6) and associative links (§6.9); performs explicit human-directed status transitions (LS5; never `superseded`, which only `kb create --supersedes` sets); sets or clears `pending_upstream` markers on deferral/propagation (ADR 0003); advances `timestamp` (and `last_human_touch` on human confirmation); appends a `revised` log entry. Validates fully before writing. Replacement stays with `kb create --supersedes`. |
...
- **ING-4.** Executes the resulting consolidation (CS6, via `kb revise`), supersession, and propagation plan per CS3, updating affected downstream documents in dependency order, respecting per-document `instructions`, and pausing for human confirmation wherever a change involves judgment. Where the human defers downstream propagation, the affected children are marked `pending_upstream` (ADR 0003) so the deferral stays visible to lint and consumers.
...
- **LINT-2.** Semantic review (LLM-level): contradictions between `current` documents, coverage gaps (DG3), orphans (no consumers and no children), possibly-outdated documents (an ancestor's `timestamp` newer than the document's, with no `pending_upstream` marker recording a deliberate deferral), and `instructions` problems (IN4).
...
- **LINT-4.** **Impact view:** for any recently superseded or revised document, the transitive downstream set and its update status. Reads `pending_upstream` markers directly, distinguishing deliberate deferrals (marked) from unexamined changes (unmarked) — surfacing propagation work that was deferred or missed. Documents connected only by associative links appear as *notified*, never obligated (ADR 0004).
...
2. **OQ2 — Draft visibility to consumers.** Should consuming agents be permitted to read `draft` documents with a warning, or be hard-blocked? Proposal: hard-block by default, governance-relaxable per type. (Distinct and already decided: a `current` document carrying `pending_upstream` is served with a **warning** naming the unpropagated parent revision, never blocked — it is complete, merely possibly outdated; ADR 0003.)
...
| **Purpose document** | A leaf synthetic document distilled from a small sub-network, carrying complete and only the context needed for one specific outcome |
| **Building block** | An intermediate synthetic document with no consumer-facing purpose of its own — a reusable node from which purpose documents are distilled |
| **KB charter** | The governance document stating the KB's purpose: who it serves, its consumers, and the architectural rationale behind structure and vocabularies — intent, not rules |
| **Conventions** | The governance document of binding authoring/maintenance rules; the top of IN1's precedence chain (rationale → charter; rules → conventions; settings → config) |
| **Associative link** | A typed, non-provenance relationship (`references`, `contradicts`, `constrains`, …) in synthetic frontmatter; navigational, never parentage (§6.9) |
| **Supersession** | Replacement or rescoping of a document by a new one, preserving lineage via `supersedes` and Git history; reserved for genuine replacement |
| **Revision** | An in-place update to a synthetic document preserving its id (`kb revise`) — the write path for consolidation and editorial change |
| **Consolidation** | Folding newly ingested evidence into an existing synthetic document it concerns, by revision (CS6) |
| **Propagation** | Human-confirmed updating of the downstream set after a supersession or revision |
| **Stale** | The condition of a `current` document whose parent's revision awaits propagation, recorded by the `pending_upstream` marker (ADR 0003) |
