...
This product is a **shared project knowledge base** in which every human role — product manager, UX designer, software engineer, infrastructure engineer, QA engineer — and every AI agent works in the **same context**. The knowledge base holds two classes of documents: **raw sources** (immutable evidence: vision documents, meeting notes, chat histories, research, compliance rules) and **synthetic documents** (human-directed, agent-maintained refinements: outcomes, design documents, specifications, test plans, runbooks, context packages — any document type a project needs). Synthetic documents form a derivation graph that progressively breaks large intent into granular, self-contained working documents; a coding specification is one possible leaf, not the privileged one.

The division of labor is the product's central idea: **humans drive creation and judgment, agents do the heavy lifting.** Humans initiate and steer the creation of every synthetic document — but *how* it is authored (live co-authoring with an agent, agent-drafted and human-approved, or another human-driven mode) is not mandated: what the KB requires is that every synthetic document derives from existing documents in the knowledge base and follows the KB's structure and rules. Agents carry the load everywhere else: drafting, analysis, consolidation, propagation, and **change recommendations submitted for human review and approval** — so humans spend their time directing, reviewing, and deciding rather than producing artifacts by hand. A document is born in a human-initiated session and is considered accepted at creation; from then on, agents keep it consistent with new evidence — consolidating new evidence into the documents it concerns (CS6), applying updates automatically where a document's own maintenance rules permit (§6.6), recommending changes for approval where judgment is needed, and **asking a human whenever anything is unclear or conflicting rather than assuming**.
...
The capability ships in two parts: a small set of **Agent Skills** (the judgment layer: how to ingest, co-author, and maintain) and a **`kb` CLI** (the deterministic layer: search, filtering, frontmatter extraction, graph queries, validation, reports), developed and versioned independently so tooling can improve without touching the skills.
...
- **G2 — Human-driven creation, agent-driven maintenance.** New synthetic documents are created only in human-initiated, human-steered workflows — the authoring method (co-authoring, agent-drafted with human approval, or otherwise) is not mandated, so long as the document derives from existing KB documents and honors the KB's schema, graph rules, and CLI write path. Agents update, propagate, reconcile, repair, and recommend changes for human approval — never originate documents autonomously.
...
- **G10 — Maximize agent leverage.** The point of giving AI agents full (CLI-mediated) access to the knowledge base is to let them do most of the heavy lifting — drafting, analysis, consolidation, propagation, recommendation — while humans concentrate on direction, review, and judgment calls. Human attention is the scarce resource; the KB exists to multiply it. Alongside G1, this is what the product is for.
...
- **NG5.** Purpose-specific context packaging (`kb-pack`) in v1. A context package is an ordinary document type a human can have authored when needed — purpose documents (G9) are exactly such human-directed packages; what v1 excludes is dedicated packaging *tooling*, not the documents.
...
**P2 — Maintenance agent (AI).** An LLM agent with this skill suite loaded, operating inside a human's session: ingesting sources, co-drafting under direction, consolidating new evidence, propagating updates, recommending changes for approval, asking questions.
...
```
 ...
 meeting notes─────► │  (immutable            (human-directed,            │
...
**CLI-exclusive access (hard rule).** Every read and every write an AI agent performs against a knowledge base goes through the `kb` CLI — agents never open, edit, or write KB files directly. `kb show` (CLI-3a) is the sole read path, and the write and housekeeping commands (CLI-7/8/13/14, CLI-9/10/11) are the sole write paths; this is what keeps access control (NFR-8), schema validity, id allocation, and graph invariants CLI-enforced rather than entrusted to model discipline. Humans may edit governance documents directly (they are human-controlled, §6.1); agents may not.
...
| **Synthetic** | `synthetic/` | Living | Human-initiated workflows (typically co-authoring), always via `kb create` | Agents, under §6.7 rules |
...
In particular, instructions may define **automatic consolidation rules**: how the agent should update this document when a new document joins its `derived_from` parents (e.g., "when a new meeting note is attached, refresh the timeline section; never touch the interface table"). Changes covered by such a rule are applied by the agent **without per-change confirmation** — logged in `log.md`, advancing `timestamp` but not `last_human_touch`, so every automatic update remains visible in the drift report (LS2) until a human confirms it. Automatic updates run through the normal CLI write path (`kb revise`), so schema validity and graph acyclicity (DG2) are still enforced on every one; anything the rule does not clearly cover falls back to a recommendation for human approval (CS2/CS6).
...
- **LS1.** A document created in a human-initiated session becomes `current` at creation — acceptance is implicit in the human's steering and approval of the session, whatever the authoring mode. The human may instead leave it `draft` (open questions pending), returning later to complete it.
...
- **LS3.** Agent edits to `current` documents occur only within the operations defined in §7's skills (ingestion propagation, instruction-authorized consolidation per §6.6, human-directed revision, lint-proposed fixes accepted by a human) — never as free-standing autonomous edits.
...
*Trigger: a human asks to create a new document of any type, or to revise an existing one. This is the standard path by which synthetic documents come into existence; whatever the authoring mode, every synthetic document is born via `kb create` in a human-initiated workflow (G2), never autonomously (NG3).*
...
- **NFR-3 — Deterministic mechanical truth.** Every property checkable without an LLM is checked by the CLI (CLI-12); governance rules about status, schema, and graph shape never depend on model judgment. All agent access to a KB, read and write alike, is CLI-mediated — direct file access by agents is prohibited (§5.2).
...
│       └── index.md
└── synthetic/                    # human-directed, agent-maintained; organization is
...
| **Synthetic document** | Human-directed, agent-maintained document derived from parents in the graph — the authoring mode is free; the provenance, schema, and graph rules are not | refined document, output, artifact |
...
| **Propagation** | Updating of the downstream set after a supersession or revision — human-confirmed for judgment calls; automatic only within governance- or instruction-authorized classes (CS3, §6.6) | cascade, sync |
| **Co-authoring** | The human-initiated, human-led session in which a synthetic document is created or revised with an agent — the standard mode of human-driven creation (G2 mandates human initiation and the CLI write path, not the interaction style) | compile, generate, ETL, pipeline |
