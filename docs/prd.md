...
- **AUT-7.** Revision mode: the same session pattern applied to an existing document at the human's request. Semantic change is human-confirmed **supersession** — a new `kb create --supersedes <old>` that atomically marks the prior document `superseded` (FM/CS rules). In-place editorial revision (body/frontmatter edits without supersession) awaits the mutation write path (OQ6).
- **AUT-8.** Session record: before closing, the agent summarizes the decisions made in the session and confirms the summary with the human, then archives the session to `raw/chats/` (via `kb ingest --class chat`) — the human-confirmed decision summary first, followed by the substantive exchange. Its id is supplied among the document's `--derived-from` at `kb create` (AUT-6), so session parentage (DG5) holds from creation; for a document created earlier in the session, adding the parent afterward requires the revision write path (OQ6). Verbatim completeness of the transcript is best-effort and platform-dependent; the decision summary is the mandatory, citable part.
...
6. **OQ6 — Synthetic revision write path.** `kb create` (CLI-13) covers document *birth*; in-place revision of an existing synthetic document — editorial body/frontmatter edits, adding a session-record parent after creation (AUT-8), status changes short of supersession — has no deterministic command yet. v1 expresses semantic revision as supersession (`kb create --supersedes`, AUT-7); a dedicated mutation command (e.g. `kb set`/`kb revise`) is deferred. Proposal: gather Phase 3 `kb-author` field data on how often in-place editorial edits are actually needed before specifying it.
