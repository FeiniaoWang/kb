...
- **Git-agnostic:** no command runs `git` or inspects Git state — the CLI only reads and writes files. The KB lives in Git, but initializing/committing/branching is the user's or a skill's job. (`.gitkeep` files are inert placeholders, not a Git operation.)
